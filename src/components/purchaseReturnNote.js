...
lDisable=false;
  ...
  const onSave = async (voucherData, drTotal, crTotal) => {
   ...
   Axios
      .post(url + '/purchaseReturnNote', data
          ...
          <tbody style={mystyle} >
            {invoiceData.map(item => {
              return <tr key={item.id}>
                ...
                <td style={{ textAlign: "right", backgroundColor: '#999999' }}>{parseFloat(item.itemTaxTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</td>
                <td style={{ textAlign: "right", backgroundColor: 'cyan' }}>{parseFloat(item.itemNetTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</td>
                <button className={'fa fa-check-square'} style={{ backgroundColor: 'green', color: 'white', border: "3px solid black", align: 'left' }} onClick={() => handleSelectProduct(item.productID, item.productName, item.taxID, item.unit, item.unitPrice, item.purchaseQty)}> </button>
            ...
            <td style={{ textAlign: "right", backgroundColor: 'cyan', color: "red" }}>{parseFloat(totalTax).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</td>

            <td style={{ textAlign: "right", color: "blue", backgroundColor: 'cyan' }}>{parseFloat(totalNetAmt).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}</td>

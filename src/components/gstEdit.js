...
class GstEdit extends React.Component {
   ...
   //   this.handleChange = this.handleChange.bind(this);
    ...
    .then(res => {
      console.log(res);
      taxData = res.data;
   //   alert(taxData[0].taxRate);
    //  alert(taxData[0].taxCode);
      this.taxIDEl.current.value = taxData[0].taxID;
      this.taxTypeEl.current.value = taxData[0].taxType;
      this.taxCodeEl.current.value = taxData[0].taxCode;
      ...
      taxrate = taxData[0].taxRate.toFixed(2);
      this.setState({number: taxrate});
     ...
     */
      ...
      return (
        ...
        <form style={mystyle} onSubmit={this.handleSubmit}>
          <fieldset>
           <label>
           <h1>Goods And Services Tax (GST) Profile Maintenance</h1>
          ...
          <td style={{color: 'black'}}>
              ...
              <td style={{color: 'red', paddingLeft: '40px'}} >
               <input type="radio" value="OUTPUT" name="taxtype" onChange={this.handleChangeType} ref={this.taxTypeEl} checked={this.state.taxtype === 'OUTPUT'} />GST Output Tax
               </td>
               <td style={{color: 'blue', paddingLeft: '10px'}}>
          ...
          <input type="number"  value={ this.state.number } defaultValue={taxrate} onChange={ this.handleInputChange }
          onBlur={ this.formatInput } maxLength={5} ref={this.taxRateEl}   name="taxrate" required = {false} />
          </label>

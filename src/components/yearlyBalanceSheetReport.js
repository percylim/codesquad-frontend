...
import BSREPORTPDF from './pdfBSReport';
...
function YearlyBalanceSheetReport() {
...
//eDate.setFullYear(finYear);
 ...
 // alert(Data[Data.length-1].totalText);
      BSREPORTPDF(companyData, Data, format(sDate, 'dd/MM/yyyy'), format(eDate, 'dd/MM/yyyy'))
    });
...
.get(url + `/yearlyBalanceSheet`,
  {
   ...
   if (res.data.length === 0) {
     alert('No Balance Sheet Record on '+year+' loaded ');
     return false;
   }
   setData(res.data);
        ...
        <div>
          ...
          <div className="row">
            ...
            <div className="col-sm-12" style={{ marginTop: '1px', backgroundColor: 'white', color: 'black' }}>
              <h2>Yearly Balance Sheet Report</h2>
            </div>
          </div>
...
export default YearlyBalanceSheetReport;

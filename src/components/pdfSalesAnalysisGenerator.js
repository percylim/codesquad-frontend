...
// for each ticket pass all its data into an array
   data.forEach(data => {
    const ticketData = [
      data.year,
      parseFloat(data.sales).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'),
...
*/
   ...
   doc.text(10, finalY+5, "Report Total :");
   if (reportType === 'MONTH')  {   
   doc.text( 102, finalY+5, totalSales, {align: 'right'});
   doc.text(129, finalY+5, totalDebit, {align: 'right'});
   doc.text(160, finalY+5, totalCredit, {align: 'right'});
   doc.text(203, finalY+5, totalNetSales, {align: 'right'})
   }else{
    doc.text( 81, finalY+5, totalSales, {align: 'right'});
    doc.text(114, finalY+5, totalDebit, {align: 'right'});
    doc.text(151, finalY+5, totalCredit, {align: 'right'});
    doc.text(203, finalY+5, totalNetSales, {align: 'right'})
 ...
 //  doc.setFontSize(12);
   ...
   addFooters(doc) ;   // page count function
    let ReportName = '';
    if (reportType === 'YEAR') {
       ReportName = 'YearlySalesAnalysis-'+todayDate;
    } else {
      ReportName = 'YearlySalesAnalysis-'+todayDate;
    }
   doc.save(ReportName);

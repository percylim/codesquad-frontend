...
import ExportSalesNotePDF from "./pdfSalesNoteGenerator";
...
function SalesDrCrNote() {
  ...
  //  const [txnType, setTxnType] = useState('DR');
    ...
    const [invType, setInvType]= useState('SCN');
        ...
        .get(url + `/companyInfo`,
          ...
          .get(url+`/taxList`,
              ...
              .then(res => {
                  ...
                  taxID=res.data[0].taxID;
                  taxType=res.data[0].taxType;
                  taxCode=res.data[0].taxCode;
                  taxRate=res.data[0].taxRate;
                  taxDescription=res.data[0].taxDescription;
                  taxRemark = res.data[0].remark;
                   setTax(res.data[0].id);
               //   alert(taxRate);
              } else {
                taxID = '';
                taxType= '';
...
const formatInputInvoiceNo = async(e) => {
  ...
  const handleAddNote = () => {
      noteDate=moment(txnDate).format('DD-MM-YYYY');
    //let date = new Date(txnDate);
    //setTxnDate(format(new Date(date), 'dd/MM/yyyy'));
    // e.preventDefault();
  //       alert(noteDate);
    //   return false;
    if (txnAmount === 0 || txnAmount === '') {
       alert("Debite/Credit Amount must enter");
       return false;
    }
...
// alert(netTotal);
       ...
       // newDatas[i].txnDate = format(new Date(txnDate), "dd/MM/yyyy");
...
*/
  ...
  const onNew = async() => {
    ...
    lDisable=false;
    setData(...data);
    window.location.href='/salesDrCrNote';
  };
...
// alert(res.data);
 if (res.data !== 'Existed') {
    alert('Sales Invoice No. '+invoiceNo+' Invalid, please re-enter')
     return false
  }
...
//alert(data[0].documentNo);
//*************************** .     */
//return false;

//alert(data[0].voucherNo);
/** update sales Invoice cr.dr note*/

Axios
.post(url+'/salesNote',data
...
/** update voucher */
//alert('Update Journal Voucher in next step : '+data.length);
// terminate temperally in this step
//return false;
...
const onSearchInvoice = async (e) => {
     ...
     //  alert(e);
      if (invNo === '' || invNo === null) {
        alert('Sales Invoice No. cannot be blank');
        return false;
      }
      ...
      //  alert(res.data);
       ...
       if (res.data === "Invalid")   {
        ...
        for (let i = 0; i < res.data.length; i++) {
            if (res.data[i].invType === 'SAL' || res.data[i].invType === 'SDN') {
                amt+=res.data[i].drAmt;
            }
            if (res.data[i].invType === 'SCR' || res.data[i].invType === 'SRN' || res.data[i].invType === 'SPY') {
              amt-=res.data[i].crAmt;
          }
       ...
       //   alert(res.data.length);
           ...
           setTaxTotal(res.data[0].taxTotal.toFixed(2));
   ...
   //     setItemNetTotal(parseFloat(netTotal).toFixed(2));
    ...
    return (
        ...
        <div>
          ...
          <div className="col-sm-12 btn btn-primary" style={{marginTop: '1px'}}>
              Sales Invoice Debit Note / Credit Note Entry
           </div>
           </div>
        ...
        <a style={{marginRight: '.3rem'}}> Transaction Date  : </a>
          <input
            type="date"
            maxLength={10}
            value={txnDate}
    ...
    <a style={{marginLeft: '4.4rem', marginRight: '.2rem'}}> Transaction Type : </a>
...
<a style={{marginRight: '2.2rem'}}> Customer ID : </a>
<input
  type="text"
  style={{width: '200px', border: '1px solid #696969'}}
  value={supplierID}
  ...
  readOnly={lDisable}
   ...
   type='button'
   class = 'btn btn-primary fa fa-search float-right'
   onClick={() => onSearch(supplierID)}
 
   ></button>

   <a style={{marginLeft: '1.4rem', marginRight: '0.1rem'}} >Customer Name : </a>
...
<label style={{ paddingLeft: "10px"}} >
<a style={{marginRight: '0.1rem'}}> Sales Invoice No. : </a>
<input
  type="text"
  style={{width: '200px', border: '1px solid #696969'}}
  value={invoiceNo}
...
<a style={{marginLeft: '1rem', marginRight: '1.9rem'}}> Invoice Total : </a>
<input
  type="number"
...
<a style={{marginLeft: '1rem', marginRight: '.1rem'}}> Debit/Creit Amount : </a>
<input
  type="number"
  style={{width: '150px', border: '1px solid #696969'}}
  value={txnAmount}
...
<label style={{paddingLeft: '10px'}}>
<a style={{marginRight: '2rem'}}> Tax Selection  : </a>
...
<input
  type="text"
  style={{marginLeft: '4rem', width: '1300px', border: '1px solid #696969' }}
  value={txnParticular}
  name="txnParticular"
...
<p></p>
        ...
        <table class="table" style={{marginLeft: '.5rem', paddingTop: '1px', border: '1px solid black'}}>
            <thead class="thead-dark" >
                ...
                <th style={{backgroundColor: 'yellow', width: '500px', textAlign: 'center'}}>Transaction Particualar</th>
                <th style={{backgroundColor: '#999999', width: '130px', textAlign: 'center' }}>Txn. Amount</th>
                <th style={{backgroundColor: 'yellow', width: '130px', textAlign: 'center'}}>Tax Amount</th>
                <th style={{backgroundColor: '#9999999', width: '130px', textAlign: 'center'}}>Net Amount</th>
                 <th style={{backgroundColor: 'blue', textAlign: 'center', color: 'white', width: '15px'}}>Action</th>
                </tr>
            </thead>
            <tbody style={mystyle} >
                {data.map(item => {
...
<td></td>
<td><button style={{backgroundColor: "green", color: "white", width: '300px', marginLeft: '.5rem'}} onClick={() => onPrintNote()}>Print Credit/Debit Note</button></td>
...
<td><button style={{backgroundColor: "cyan", color: "black", width: '200px'}} onClick={() => handleAddNote()}>Add Dr/Cr Note</button></td>
...
<p></p>
           ...
           <td><button style={{backgroundColor: "red", color: "white", width: '300px'}} onClick={() => onSave(voucherData, totalDrAmt, totalCrAmt)}>Save Invoice and Voucher</button></td>
           <td><button style={{backgroundColor: "cyan", color: "black", width: '250px'}} onClick={() => onAddVoucher()}>Add Voucher Item</button></td>

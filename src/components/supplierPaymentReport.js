...
var  payData = [];
var newDatas = [];
var fName = "Supplier payment Report for ";
...
function SupplierPaymentReport() {
  ...
  const [invType, setInvType]= useState('PPY');
  const mystyle = {
    align: "left",
  ...
  const columns = [
...

    {
      ...
      formatter: (cellContent: string, row: IMyColumnDefinition) => {
        ...
        return <button className="fa fa-check-square" onClick={() => handleSelectSupplier(row.supplierID, row.supplierName)}></button>
  ...
  const handleChangeType= async(e) => {
   ...
   //   defaultInvType=e.target.value;
      // noteType = 'Credit Note';
       if (e.target.value === 'PPY') {
        fName = "Supplier payment Report for " ;
       }
       if (e.target.value === 'PCN') {
        fName = "Supplier Credit Note Report for " ;
       }
       if (e.target.value === 'PDN') {
        fName = "Supplier Debit Note Report for " ;
       }
       if (e.target.value === 'PRN') {
        fName = "Supplier Return Note Report for " ;
       }
  ...
  const handleNew = async () => {
    ...
    window.location.href = '/supplierPaymentReport';
...
const handleSelectSupplier = async (e,n) => {
// alert(e);
//return false;
//alert(sDate);
//alert(eDate);
 ...
 setSupplierID(e);
  ...
  //  setCompanyInfo(res.data);
      if (res.data.length ===0) {
        alert('No Purchase Invoice '+cType+' between '+moment(startDate).format("DD/MM/YYYY")+' and '+moment(endDate).format("DD/MM/YYYY"));
      } else {
...
const csvReport = {
  ...
  data: invoiceTxn,
  headers: headers,
  filename: 'Purchase-payment-report-'+supplierID+'-from-'+txnDate+'-to-'+dueDate+'.csv',
  ignoreFooter: false
};
/*
const csvData = [
...
*/
  ...
  return (
    ...
    <div>
      ...
      <div className="row">
        <div className="col-sm-12" style={{backgroundColor: 'purple', color: 'white', marginTop: '1px' }}>
          Supplier Invoice Payment/Debit/Credit/Goods Return Report
        </div>
      </div>
    ...
    >
     <p></p>
      ...
      <a style={{  marginRight: '.8rem' }} >Purchase Invoice Payment From Date : </a>
        <input
          type="date"
          maxLength={10}
          value={txnDate}
...
<div>
<label style={{ paddingLeft: "10px" }} >
<a style={{ marginRight: '.2rem' }}> Supplier ID : </a>
        <input
          type="text"
...
<a style={{ marginLeft: '3.2rem', marginRight: '.2rem' }}> Supplier Name : </a>
        <input
          type="text"
...
<div className="col-sx-10 btn btn-warning" style={{ marginTop: '1px', paddingLeft: '10px' }}>
  Purchase Invoice Payment Transaction Details
</div>
</div>

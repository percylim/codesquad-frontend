...
const ExportInvoicePDF = (data, companyInfo, customerInfo, invoiceType, invoiceTotal, invoiceDiscountTotal, invoiceTaxTotal, invoiceNetTotal)=> {
...
//  for(var i = 1; i <= pageCount; i++) {
 ...
 var  addFooters = doc => {
  ...
  const tableColumn = ["NO.", "DESCRIPTION", "QTY", "UNIT", "PRICE", "AMT", "DIS", "TAX", "NET AMT"];
  ...
  const salesRep = data[0].salesRep;
  const remark1 = data[0].remark1;
...
const totalInvoice=parseFloat(invoiceTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
const totalDiscount=parseFloat(invoiceDiscountTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
const totalTax=parseFloat(invoiceTaxTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
const totalNet=parseFloat(invoiceNetTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
...
// for each ticket pass all its data into an array
   data.forEach(data => {
    const ticketData = [
      ...
      data.unit,
      parseFloat(data.unitPrice).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'),
      parseFloat(data.itemTotal).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'),
      parseFloat(data.itemDiscount).toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'),
...
*/
   ...
   doc.text(203, finalY+5, totalInvoice, {align: 'right'});

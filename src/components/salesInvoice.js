...
//       alert(typeof totalCrAmt);
 ...
 if (lOk === false) {
   ...
   return false;
    ...
    Axios
      ...
      .then(res => {
        ...
        // alert(res.data);
        if (res.data === 'Existed') {
          alert('Sales Invoice No. ' + invoiceNo + ' already Existed, please re-enter')
          return false
        }
  ...
  //  if (invoiceNo === '' || invoiceNo === null) {
      Axios.get(url + '/lastSalesInvoice',
       {
         params: {
           companyID: companyID,
           jvInit: cYear,
     ...
     .then(res => {
      ...
      //   alert(res.data[0].invoiceNo);
     //    alert(res.data[0].remark1);
        setInvoiceNo(iNo);
       // alert(res.data[0].remark1 || res.data[0].remark1 === null);
         // check if remark1 - 6 is empty than use defauld
...
<a style={{ marginLeft: '1rem', marginRight: '.8rem' }} >Due Date : </a>
        ...
        <a style={{ marginLeft: '1rem', marginRight: '.2rem' }}> Sales Rep. : </a>
        <input
          type="text"
...
/>
        ...
        <td><button style={{ backgroundColor: "green", color: "white", width: '250px', marginLeft: '15rem' }} onClick={() => onPrintSalesInvoice()}>Print Sales Invoice </button></td>
